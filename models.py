...
class Supplier(Base):
    ...
    )  # Whether this supplier has been embedded to embeddings database
...
class Product(Base):
    ...
    )  # Number of units in a package (e.g., 1000 pieces per package)
    ...
    )  # Cached calculated price for performance
    calculated_price_updated_at = Column(
        DateTime(timezone=True), nullable=True
    )  # When calculated price was last updated
    embedded = Column(
        Boolean, default=False, nullable=False
    )  # Whether this product has been embedded to embeddings database
...
class SupplierProduct(Base):
    ...
    )  # Keep for now, will be removed later
    ...
    default_margin = Column(
        Numeric(5, 4), nullable=True
    )  # Margin as decimal (0.25 = 25%). Formula: price = cost / (1 - margin)
    currency = Column(
        String(3), default="MXN", nullable=False
    )  # Currency of cost and shipping costs (MXN or USD)
    stock = Column(Integer, default=0)
    lead_time_days = Column(Integer, nullable=True)
    shipping_cost = Column(
        Numeric(10, 2), nullable=True
    )  # Legacy shipping cost (deprecated)
    shipping_cost_direct = Column(
        Numeric(10, 2), default=0.00, nullable=False
    )  # Direct shipping cost per unit
    shipping_method = Column(
        String(20), default="DIRECT", nullable=False
    )  # DIRECT or OCURRE shipping method
    ...
    embedded = Column(
        Boolean, default=False, nullable=False
    )  # Whether this supplier product has been embedded to embeddings database
    embedding = Column(Vector(1536))  # Embedding vector for semantic search
...
class KitItem(Base):
    ...
    supplier_product_id = Column(
        Integer, ForeignKey("supplier_product.id", ondelete="CASCADE")
...
class Balance(Base):
    ...
    description = Column(Text, nullable=True)
    balance_type = Column(
        String(20), default="QUOTATION", nullable=False
    ...
    items = relationship(
        "BalanceItem", back_populates="balance", cascade="all, delete-orphan"
    )
...
class BalanceItem(Base):
    ...
    supplier_id = Column(
        Integer, ForeignKey("supplier.id", ondelete="CASCADE")
    )  # Keep for backward compatibility
    supplier_product_id = Column(
        Integer, ForeignKey("supplier_product.id", ondelete="CASCADE")
    )  # NEW - primary reference
    ...
    )  # (unit_price + calculated_shipping) * quantity
    notes = Column(Text, nullable=True)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class Query(Base):
    ...
    retrieved_chunk_ids = Column(
        JSON, nullable=True
    )  # Pinecone vector ids injected into the prompt
...
class WAMessage(Base):
    ...
    conversation_id = Column(
        Integer,
        ForeignKey("wa_conversation.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    wa_message_id = Column(
        String(128), unique=True, nullable=True
    )  # WhatsApp's id (dedup inbound)
    ...
    )  # received, draft, approved, sent, delivered, read, failed
...
class WADraft(Base):
    ...
    trigger_message_id = Column(
        Integer, ForeignKey("wa_message.id", ondelete="SET NULL"), nullable=True
    )
    draft_text = Column(Text, nullable=False)
    edited_text = Column(Text, nullable=True)  # operator's edit before sending
    ai_context = Column(
        Text, nullable=True
    )  # product context used (operator visibility)
    status = Column(
...
class RoadmapItem(Base):
    ...
    )  # 0 = already shipped, 1..5 = roadmap phases
...
class Quotation(Base):
    ...
    user_query = Column(
        Text, nullable=False
    )  # Original query that generated the quotation
    ...
    customer_quotation = Column(
        Text, nullable=False
    )  # Customer-facing quotation markdown
...
class SocialPost(Base):
    ...
    date_for = Column(
        Date, nullable=False
    )  # Target date (migrated from VARCHAR to DATE)
    caption = Column(Text, nullable=False)
    image_prompt = Column(Text, nullable=True)
    post_type = Column(String(50), nullable=True)
    content_tone = Column(
        String(50), nullable=True, index=True
    )  # Content tone: Motivational, Technical, Humor, Educational, Inspirational, etc.
    ...
    formatted_content = Column(
        JSON, nullable=True
    )  # Full JSON response (migrated to JSONB in DB)
    external_id = Column(
        String(255), nullable=True, index=True
    )  # External ID for efficient lookups (e.g., from formatted_content.id)
    # Channel-specific fields
    channel = Column(String(50), nullable=True)  # wa-status, fb-post, tiktok, etc.
    carousel_slides = Column(
        JSON, nullable=True
    )  # Array of slide prompts for carousels (TikTok, FB/IG)
    needs_music = Column(
        Boolean, default=False
    )  # Whether this content needs background music
    ...
    topic = Column(
        Text, nullable=False
    )  # Topic in format "Problema → Solución" (NOT NULL after migration)
    problem_identified = Column(
        Text, nullable=True
    )  # Problem description from strategy phase
    topic_hash = Column(
        String(64), nullable=False, index=True
    )  # SHA256 hash of normalized topic (NOT NULL after migration) - used to detect same topic on same date
    created_at = Column(DateTime(timezone=True), server_default=func.now())
...
class FileMetadata(Base):
    ...
    supplier_id = Column(
        Integer, ForeignKey("supplier.id", ondelete="SET NULL"), nullable=True
    )
    quotation_id = Column(
        Integer, ForeignKey("quotation.id", ondelete="SET NULL"), nullable=True
    )
    task_id = Column(
        Integer, nullable=True
    )  # FK constraint added at DB level (task table is in impag-tasks)
...
class DocumentChunk(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    file_id = Column(
        Integer, ForeignKey("file_metadata.id", ondelete="CASCADE"), nullable=False
    )
...
class LogisticsMetadata(Base):
    ...
    supplier_product_id = Column(
        Integer, ForeignKey("supplier_product.id", ondelete="SET NULL"), nullable=True
    )
    supplier_id = Column(
        Integer, ForeignKey("supplier.id", ondelete="SET NULL"), nullable=True
    )
...
class Task(Base):
    ...
    creator = relationship(
        "TaskUser", foreign_keys=[created_by], back_populates="tasks_created"
    )
    assignee = relationship(
        "TaskUser", foreign_keys=[assigned_to], back_populates="tasks_assigned"
    )
...
def get_current_task_user(db, email: str):
    ...
    return (
        db.query(TaskUser)
        .filter(TaskUser.email == email, TaskUser.is_active == True)
        .first()
    )
...
def get_next_task_number(db):
    ...
    used = {
        r[0]
        for r in db.query(Task.task_number).filter(Task.task_number.isnot(None)).all()
    }
    n = 1
    while n in used:
        n += 1
...
class QuoteItem(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    quote_id = Column(
        Integer, ForeignKey("quote.id", ondelete="CASCADE"), nullable=False
    )
    product_id = Column(
        Integer, ForeignKey("product.id"), nullable=True
    )  # null for freeform items
    supplier_product_id = Column(
        Integer, ForeignKey("supplier_product.id"), nullable=True
    )

    description = Column(
        String(500), nullable=False
    )  # product name or freeform description
    sku = Column(String(100), nullable=True)
